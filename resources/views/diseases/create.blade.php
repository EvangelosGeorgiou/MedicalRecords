        ...
        <form action="{{ isset($disease) ? route('diseases.update', $disease->id) : route('diseases.store') }}"
            ...
            <div class="container">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="doc_name" id="doc_name" class="form-control" placeholder="Enter doctor name" value="{{ isset($disease) ? $disease->doc_name : '' }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="icd_code_id" id="icd_code" class="form-control">
                        ...
                        @if(isset($disease))
                            <option value="{{ $disease->icdCodeInfo['id'] }}">{{ $disease->icdCodeInfo['icd_code'] }} / {{ $disease->icdCodeInfo['name'] }}</option>
                ...
                <div class="form-group">
                    ...
                    <input  type="hidden" name="description" id="description" value="{{ isset($disease) ? $disease->description : '' }}">
                    <trix-editor input="description"></trix-editor>
                </div>

                <div class="form-group">
                    <label for="diagnosis">Diagnosis</label>
                    <input  type="hidden" name="diagnosis" id="diagnosis" value="{{ isset($disease) ? $disease->diagnosis : '' }}">
                    <trix-editor input="diagnosis"></trix-editor>
                </div>

                <div class="form-group">
                    <label for="symptoms">Symptoms</label>
                    <input  type="hidden" name="symptoms" id="symptoms" value="{{ isset($disease) ? $disease->symptoms : '' }}">
                    <trix-editor input="symptoms"></trix-editor>
                </div>

                <div class="form-group">
                    <label for="date">Date </label>
                    <input type="date" class="form-control" name="date" id="date" value="{{ isset($disease) ? $disease->date : '' }}">
                </div>
...
{{-- ICD CODE SEARCH FUNCTIONALLITY --}}
<script>
    $(document).ready(function(){

     $('.icd_code_id').keyup(function(){
            var query = $(this).val();
            if(query != '')
            {
             var _token = $('input[name="_token"]').val();
             $.ajax({
              url:"{{ route('disorders_icd.fetch', $patients->id) }}",
              method:"POST",
